...
import DeviceDataDisplay from "@/components/DeviceDataDisplayPanasonic";
  ...
  const handleMessage = (topic: string, message: Buffer) => {
    try {
      ...
      if (topic === "response_device_modbus") {
        ...
        // Filter out status messages (strings) and only keep actual device data payloads
        if (typeof payload === 'object' && payload !== null && !Array.isArray(payload)) {
          // Only accept payloads with device_name and value fields (actual battery data)
          if (payload.device_name && payload.value) {
            setDeviceTopicData((prev) => ({ ...prev, [topic]: payload }));
          }
          // Status messages like "Shoto_Battery_1 data acquisition success" are ignored
        }
      }
    } catch (e) {
