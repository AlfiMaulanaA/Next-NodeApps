...
export default function MqttDiscoveryPage() {
  ...
  const [presetsManageDialogOpen, setPresetsManageDialogOpen] = useState(false);
  ...
  useEffect(() => {
    const savedBrokerConfig = localStorage.getItem('mqtt-discovery-broker-config');
    const savedSubscriptions = localStorage.getItem('mqtt-discovery-subscriptions');
    const savedPresets = localStorage.getItem('mqtt-discovery-presets');
    
    if (savedBrokerConfig) {
      try {
        setBrokerConfig(JSON.parse(savedBrokerConfig));
      } catch (e) {
        console.warn('Failed to parse saved broker config');
      }
    }
    ...
    if (savedSubscriptions) {
      try {
        ...
        console.warn('Failed to parse saved subscriptions');
      }
    }
    
    if (savedPresets) {
      try {
        setBrokerPresets(JSON.parse(savedPresets));
      } catch (e) {
        console.warn('Failed to parse saved presets');
      }
    }
  }, []);
  ...
  useEffect(() => {
    localStorage.setItem('mqtt-discovery-broker-config', JSON.stringify(brokerConfig));
  }, [brokerConfig]);
  
  useEffect(() => {
    localStorage.setItem('mqtt-discovery-subscriptions', JSON.stringify(subscriptions));
  }, [subscriptions]);
  
  useEffect(() => {
    localStorage.setItem('mqtt-discovery-presets', JSON.stringify(brokerPresets));
  }, [brokerPresets]);
  
  // Topic validation
  const validateTopicPattern = useCallback((topic: string): boolean => {
    if (!topic || topic.length === 0) return false;
    
    // MQTT topic rules:
    // - Cannot contain null characters
    // - Cannot be longer than 65535 bytes
    // - + is single level wildcard (must be alone in level)
    // - # is multi-level wildcard (must be at end)
    ...
    for (let i = 0; i < levels.length; i++) {
      ...
      if (level.includes('+')) {
        if (level !== '+') return false; // + must be alone in level
      }
      
      // Multi-level wildcard rules
      if (level.includes('#')) {
        if (level !== '#') return false; // # must be alone in level
        if (i !== levels.length - 1) return false; // # must be at end
      }
    }
  ...
  useEffect(() => {
    if (isDiscovering && sessionStats.startTime) {
      statsIntervalRef.current = setInterval(() => {
        const currentMessageCount = messages.length;
        const messagesSinceLastCheck = currentMessageCount - lastMessageCountRef.current;
        const rate = messagesSinceLastCheck; // per second
        ...
        setSessionStats(prev => ({
          ...
          messageRate: rate,
          peakMessageRate: Math.max(prev.peakMessageRate, rate),
          bytesReceived: messages.reduce((sum, msg) => sum + msg.payload.length, 0)
        }));
  ...
  }, [isDiscovering, sessionStats.startTime, messages.length, topicsData.size, messages]);
  ...
  }, [brokerConfig, subscriptions, maxMessages, attemptReconnection]);
  ...
  }, [mqttClient, isConnected, messages.length]);
  ...
  const handleAddSubscription = useCallback(() => {
    const topic = newSubscription.topic.trim();
    if (!topic) {
    ...
    if (!validateTopicPattern(topic)) {
      toast.error("Invalid topic pattern. Check MQTT topic rules.");
      return;
    }

    addSubscription(topic, newSubscription.qos);
    setNewSubscription({ topic: "", qos: 0 });
    setAddSubscriptionDialogOpen(false);
  }, [newSubscription, addSubscription, validateTopicPattern]);
  ...
  const loadPreset = useCallback((presetId: string) => {
    const preset = brokerPresets.find(p => p.id === presetId);
    if (preset) {
      setBrokerConfig(preset.config);
      setTempBrokerConfig(preset.config);
  ...
  const saveAsPreset = useCallback((name: string) => {
    const newPreset: BrokerPreset = {
      id: Date.now().toString(),
      name,
      config: { ...brokerConfig }
    };
    
    setBrokerPresets(prev => [...prev, newPreset]);
    toast.success(`Preset saved: ${name}`);
  }, [brokerConfig]);
  
  // Delete preset
  const deletePreset = useCallback((presetId: string) => {
    setBrokerPresets(prev => prev.filter(p => p.id !== presetId));
  ...
  return (
    <SidebarInset>
      <header className="flex h-16 items-center justify-between border-b px-4">
        ...
        <div className="flex items-center gap-2">
          <Badge variant="outline" 
            className={`flex items-center gap-2 ${connectionStatus === 'connected' ? 'bg-green-100 text-green-800 border-green-200 dark:bg-green-900 dark:text-green-200 dark:border-green-800' : ''}`}>
            {renderStatusIcon()}
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="w-full justify-start">
            ...
            <TabsTrigger value="analytics">
              <Database className="h-4 w-4 mr-1" /> Analytics
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="analytics">
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  <Database className="h-5 w-5" /> Session Analytics
                </CardTitle>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                  <div className="bg-blue-50 dark:bg-blue-950 p-4 rounded-lg">
                    <div className="text-2xl font-bold text-blue-600">{sessionStats.totalMessages}</div>
                    <div className="text-sm text-blue-500">Total Messages</div>
                  </div>
                  <div className="bg-green-50 dark:bg-green-950 p-4 rounded-lg">
                    <div className="text-2xl font-bold text-green-600">{sessionStats.totalTopics}</div>
                    <div className="text-sm text-green-500">Unique Topics</div>
                  </div>
                  <div className="bg-purple-50 dark:bg-purple-950 p-4 rounded-lg">
                    <div className="text-2xl font-bold text-purple-600">{sessionStats.messageRate}/s</div>
                    <div className="text-sm text-purple-500">Current Rate</div>
                  </div>
                  <div className="bg-orange-50 dark:bg-orange-950 p-4 rounded-lg">
                    <div className="text-2xl font-bold text-orange-600">{sessionStats.peakMessageRate}/s</div>
                    <div className="text-sm text-orange-500">Peak Rate</div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <div className="text-sm space-y-1">
                      <div>Start Time: {sessionStats.startTime ? sessionStats.startTime.toLocaleString() : 'Not started'}</div>
                      <div>End Time: {sessionStats.endTime ? sessionStats.endTime.toLocaleString() : 'In progress'}</div>
                      <div>Duration: {sessionStats.startTime ? 
                        `${Math.floor((Date.now() - sessionStats.startTime.getTime()) / 1000)}s` : 
                        'N/A'
                      }</div>
                      <div>Data Received: {(sessionStats.bytesReceived / 1024).toFixed(2)} KB</div>
                    </div>
                  </div>
                  
                  <div className="space-y-2">
                    <h3 className="font-semibold">Top Active Topics</h3>
                    <div className="text-sm space-y-1 max-h-32 overflow-y-auto">
                      {Array.from(topicsData.values())
                        .sort((a, b) => b.messageCount - a.messageCount)
                        .slice(0, 5)
                        .map((topic, i) => (
                          <div key={i} className="flex justify-between">
                            <span className="truncate mr-2">{topic.topic}</span>
                            <span className="text-muted-foreground">{topic.messageCount}</span>
                          </div>
                        ))}
                    </div>
                  </div>
            ...
            <div>
              ...
              <div className="flex gap-2">
                ...
                <Button
                  type="button"
                  variant="outline"
                  onClick={() => setPresetsManageDialogOpen(true)}
                >
          ...
          <div className="space-y-4">
            <div>
              ...
              <Input
                ...
                className={newSubscription.topic && !validateTopicPattern(newSubscription.topic) ? 
                  "border-red-500 focus:border-red-500" : ""
                }
              />
              <div className="text-xs text-muted-foreground mt-1">
                Use + for single level wildcard, # for multi-level wildcard
                {newSubscription.topic && !validateTopicPattern(newSubscription.topic) && (
                  <div className="text-red-500 mt-1">⚠️ Invalid topic pattern</div>
                )}
              </div>
            </div>
            <div>
      ...
      <Dialog open={presetsManageDialogOpen} onOpenChange={setPresetsManageDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Edit className="h-5 w-5" />
              Manage Broker Presets
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <ScrollArea className="h-[300px]">
              <div className="space-y-3">
                {brokerPresets.map((preset) => (
                  <div key={preset.id} className="border rounded-lg p-4 hover:bg-muted/30 transition-colors">
                    <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="text-sm text-muted-foreground">
                      <div>URL: {preset.config.url}</div>
                      <div>Client ID: {preset.config.clientId}</div>
                      {preset.config.username && (
                        <div>Username: {preset.config.username}</div>
                      )}
                    </div>
                  </div>
                ))}
                {brokerPresets.length === 0 && (
                  <p className="text-center text-muted-foreground py-8">
                    No presets saved yet. Save your current broker configuration as a preset.
                  </p>
                )}
              </div>
            </ScrollArea>
          ...
          <DialogFooter>
            <Button onClick={() => setPresetsManageDialogOpen(false)}>
              Close
            </Button>
          </DialogFooter>
        </DialogContent>
