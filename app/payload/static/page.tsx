  ...
  const renderFields = () => {
    return formFields.map((f, i) => (
      <div key={i} className="flex gap-2 items-center mb-2">
        ...
        <Input
          placeholder={`Value (${f.type})`}
          value={f.value}
          onChange={(e) => updateField(i, "value", e.target.value)}
        />
        <Button variant="destructive" onClick={() => removeField(i)}>
          ×
        </Button>
      </div>
    ));
    ...
    <DialogContent>
      ...
      <div className="flex gap-2 mt-2">
        <Input
          ...
          onChange={(e) =>
            setFormMeta({ ...formMeta, interval: Number(e.target.value) })
          }
        />
        <select
          value={formMeta.qos}
      ...
      {renderFields()}
      <Button size="sm" variant="default" onClick={addField}>
        Add Field
      </Button>
      <DialogFooter>
...
export default function StaticPayloadPage() {
      ...
      try {
        ...
        if (topic === "response/data/payload") {
          if (Array.isArray(msg)) {
            ...
            console.warn(
              "Unexpected message format on response/data/payload:",
              msg
            );
          }
        } else if (
  ...
  const handleUpdateSubmit = (
    ...
    send(
      "updateData",
      { ...meta, data: dataToSend, topic: meta.topic },
    ...
    <SidebarInset>
      <header className="flex h-16 items-center justify-between border-b px-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button size="sm" onClick={openCreateModal} disabled={isLoading}>
                  ...
                  <TableRow>
                    <TableCell colSpan={5} className="text-center py-12">
                      ...
                      <p className="text-muted-foreground mb-4">
                        No payload data received yet. Click refresh to fetch the
                        data.
                      </p>
                    ...
                    <TableRow key={it.topic}>
                      ...
                      <TableCell>
                        <ul>
                          ...
                          <li>
                            LWT{" "}
                            <Badge variant={it.lwt ? "default" : "destructive"}>
                              {String(it.lwt)}
                            </Badge>
                          </li>
                          <li>
                            Retain{" "}
                            <Badge
                              variant={it.retain ? "default" : "destructive"}
                            >
                              {String(it.retain)}
                            </Badge>
                          </li>
