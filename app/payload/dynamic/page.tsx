...
export default function CustomPayloadPage() {
  ...
  const fetchAvailableDevices = useCallback(() => {
    if (clientRef.current?.connected) {
      ...
      clientRef.current.publish(
        "config/device_info",
        JSON.stringify({ command: "getDeviceInfo" })
      );
    } else {
  ...
  const handleMessage = useCallback(
      ...
      try {
        ...
        if (topic.endsWith("/device_info/response")) {
          ...
          if (Array.isArray(msg)) {
            ...
            console.warn(
              "Received unexpected format for device_info/response:",
              msg
            );
          }
          ...
          if (msg && msg.groups && Array.isArray(msg.groups)) {
            ...
            toast[isSuccess ? "success" : "error"](
              msg.message || `Operation ${msg.status}.`
            );
    ...
    [fetchSummaryData]
  ); // fetchSummaryData is a dependency because it's called inside handleMessage
      ...
      .then((res) => {
        ...
        allDevices.forEach((device: any) => {
          if (device.part_number && device.data && Array.isArray(device.data)) {
            keys[device.part_number] = device.data.map(
  ...
  useEffect(() => {
    ...
    if (!clientRef.current) {
      ...
      if (clientRef.current.connected) {
        console.log(
          "MQTT client already connected. Triggering initial data fetch."
        );
      ...
      setDevicesInForm(
        g.included_devices.map((d) => {
          ...
          return {
            ...
            keyOptions:
              deviceKeyOptionsMap.current[fullDeviceInfo?.part_number || ""] ||
              [],
            value_keys: Object.entries(d.value_keys).map(([key, value]) => ({
              key,
              value,
            })),
          };
  ...
  const validateConfiguration = (config: any): string[] => {
    ...
    if (!config.summaryTopic?.trim()) {
      errors.push("Summary topic is required");
    }

    if (!config.devicesInForm?.length) {
      errors.push("At least one device must be included");
    }

    config.devicesInForm?.forEach((device: any, index: number) => {
      if (!device.name?.trim()) {
        errors.push(`Device ${index + 1}: Name is required`);
      }

      if (!Object.keys(device.value_keys || {}).length) {
        errors.push(`Device ${index + 1}: At least one value key is required`);
      }

      // Validate value keys
      Object.entries(device.value_keys || {}).forEach(
        ([key, value]: [string, any], keyIndex: number) => {
          if (!key?.trim()) {
            errors.push(
              `Device ${index + 1}, Key ${keyIndex + 1}: Key name is required`
            );
          }
          if (!value?.trim()) {
            errors.push(
              `Device ${index + 1}, Key ${keyIndex + 1}: Value name is required`
            );
          }
        }
      );
    ...
    config.calculations?.forEach((calc: any, index: number) => {
      if (!calc.operation) {
        errors.push(`Calculation ${index + 1}: Operation is required`);
      }

      if (!calc.name?.trim()) {
        errors.push(`Calculation ${index + 1}: Name is required`);
      }

      if (!calc.value_group_selected) {
        errors.push(`Calculation ${index + 1}: Value group is required`);
      }
    });

    if (config.interval < 1 || config.interval > 3600) {
      errors.push("Interval must be between 1 and 3600 seconds");
    }
  ...
  const handleSaveGroup = () => {
    ...
    const validationErrors = validateConfiguration(configToValidate);
    ...
    const payloadData: SummaryGroup = {
      ...
      included_devices: devicesInForm.map((d) => ({
        name: d.name,
        value_group: d.value_group,
        value_keys: Object.fromEntries(
          d.value_keys.map((kv) => [kv.key, kv.value])
        ),
      })),
    ...
    toast.info(
      `Sending ${editIndex === null ? "create" : "update"} command...`
    );
  ...
  const handleDeviceSelectionInForm = (
    index: number,
    selectedDeviceName: string
  ) => {
    const updatedDevices = [...devicesInForm];
    const selectedDeviceInfo = devicesInfo.find(
      (d) => d.name === selectedDeviceName
    );
    ...
    if (selectedDeviceInfo) {
      updatedDevices[index] = {
        ...
        keyOptions:
          deviceKeyOptionsMap.current[selectedDeviceInfo.part_number] || [],
        value_keys: [{ key: "", value: "" }],
      };
    } else {
      updatedDevices[index] = {
  ...
  const handleRemoveValueKeyFromDevice = (
    deviceIndex: number,
    keyIndex: number
  ) => {
    const updatedDevices = [...devicesInForm];
  ...
  const { searchQuery, setSearchQuery, filteredData } = useSearchFilter(
    config?.groups || [],
    ["summary_topic", "included_devices.name"]
  );
  ...
  const uniqueValueGroups = Array.from(
    new Set(devicesInForm.map((d) => d.value_group).filter(Boolean))
  ) as string[];
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div className="text-center p-4 bg-muted/50 rounded-lg">
                ...
                <div className="text-2xl font-bold">
                  {config?.groups?.reduce(
                    (sum, g) => sum + g.included_devices.length,
                    0
                  ) || 0}
                </div>
                <div className="text-sm text-muted-foreground">
              ...
              <div className="text-center p-4 bg-muted/50 rounded-lg">
                ...
                <div className="text-2xl font-bold">
                  {config?.groups?.filter((g) => g.retain).length || 0}
                </div>
              ...
              <div className="text-center p-4 bg-muted/50 rounded-lg">
                ...
                <div className="text-2xl font-bold">
                  {config?.groups?.reduce(
                    (sum, g) => sum + (g.calculations?.length || 0),
                    0
                  ) || 0}
                </div>
                <div className="text-sm text-muted-foreground">
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead
                    onClick={() =>
                      handleSort("summary_topic" as keyof SummaryGroup)
                    }
                    className="cursor-pointer select-none"
                  >
                    Topic{" "}
                    <ArrowUpDown className="inline w-4 h-4 ml-1 align-middle" />
                  </TableHead>
                  <TableHead>Devices</TableHead>
                  <TableHead
                    onClick={() => handleSort("retain" as keyof SummaryGroup)}
                    className="cursor-pointer select-none"
                  >
                    Retain{" "}
                    <ArrowUpDown className="inline w-4 h-4 ml-1 align-middle" />
                  ...
                  <TableHead
                    onClick={() => handleSort("qos" as keyof SummaryGroup)}
                    className="cursor-pointer select-none"
                  >
                    QoS{" "}
                    <ArrowUpDown className="inline w-4 h-4 ml-1 align-middle" />
                  ...
                  <TableHead
                    onClick={() => handleSort("interval" as keyof SummaryGroup)}
                    className="cursor-pointer select-none"
                  >
                    Interval{" "}
                    <ArrowUpDown className="inline w-4 h-4 ml-1 align-middle" />
                      ...
                      <TableCell>
                          ...
                          {g.included_devices.map((d, devIdx) => (
                            <li key={`${g.summary_topic}-${d.name}-${devIdx}`}>
                              ...
                              <ul className="ml-4 list-none">
                                {Object.entries(d.value_keys).map(
                                  ([key, value]) => (
                                    <li
                                      key={`${g.summary_topic}-${d.name}-${key}`}
                          ...
                          <div className="mt-2 text-xs">
                            ...
                            <ul className="ml-2 list-disc list-inside">
                              {g.calculations.map((calc, calcIdx) => (
                                <li key={calcIdx}>
                                  {calc.operation} ({calc.value_group_selected})
                                  as {calc.name}
                                </li>
                              ))}
                            </ul>
                        ...
                        {g.calculation_only && (
                          <div className="mt-1 text-xs text-blue-600">
                            ...
                            calculated values will be published.
                          </div>
                        )}
                      </TableCell>
                  ...
                  <TableRow>
                    <TableCell colSpan={6} className="text-center py-12">
                      ...
                      <p className="text-muted-foreground mb-4">
                        No custom payload configurations found. Create your
                        first configuration to get started.
                      </p>
        ...
        <Dialog open={modalOpen} onOpenChange={setModalOpen}>
          <DialogContent className="max-w-3xl overflow-y-auto max-h-[90vh]">
            <DialogHeader>
              ...
              <DialogDescription>
                Configure custom MQTT payloads by defining topics, QoS,
                retention settings, and including devices with specific value
                keys. You can also define calculations.
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4 py-2">
              <Card>
                <CardHeader>
                  <CardTitle className="text-md">Payload Details</CardTitle>
                </CardHeader>
                  ...
                  {devicesInForm.map((device, deviceIndex) => (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        {devicesInForm.length > 1 && (
                          <Button
                            ...
                            size="sm"
                            onClick={() =>
                              handleRemoveDeviceFromForm(deviceIndex)
                            }
                          >
                            Remove Device
                          </Button>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div className="flex flex-col">
                          <label
                            htmlFor={`device-select-${deviceIndex}`}
                            className="text-sm font-medium mb-1"
                          >
                            Select Device
                          </label>
                          <Select
                            value={device.name}
                            onValueChange={(value) =>
                              handleDeviceSelectionInForm(deviceIndex, value)
                            }
                          >
                            <SelectTrigger
                              id={`device-select-${deviceIndex}`}
                              className="w-full"
                            >
                        ...
                        <div className="flex flex-col">
                          <label
                            htmlFor={`value-group-${deviceIndex}`}
                            className="text-sm font-medium mb-1"
                          >
                      ...
                      <h5 className="text-sm font-semibold mt-3">
                        Value Keys:
                      </h5>
                      {device.value_keys.length === 0 && (
                        <p className="text-center text-muted-foreground text-xs">
                          No keys added for this device.
                        </p>
                      )}
                      {device.value_keys.map((keyValue, keyIndex) => (
                        <div key={keyIndex} className="flex gap-2 items-center">
                            ...
                            <SelectContent>
                              <SelectGroup>
                                {device.keyOptions.length === 0 ? (
                                  <SelectLabel>
                                    No keys available for this device.
                                  </SelectLabel>
                                ) : (
                                  device.keyOptions.map((k) => (
                          ...
                          <Button
                            ...
                            size="icon"
                            onClick={() =>
                              handleRemoveValueKeyFromDevice(
                                deviceIndex,
                                keyIndex
                              )
                            }
                      ...
                      <Button
                        size="sm"
                        variant="secondary"
                        onClick={() => handleAddValueKeyToDevice(deviceIndex)}
                        className="mt-2"
                      >
              ...
              <Card>
                <CardHeader className="flex flex-row justify-between items-center">
                  ...
                  <Button
                    ...
                    onClick={() =>
                      setCalculations([
                        ...calculations,
                        { operation: "", name: "", value_group_selected: "" },
                      ])
                    }
                  >
                ...
                <CardContent className="space-y-4">
                  {calculations.length === 0 && (
                    <p className="text-center text-muted-foreground">
                      No calculations added. Click "Add Calculation" to start.
                    </p>
                  )}
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <Button
                          ...
                          onClick={() =>
                            setCalculations(
                              calculations.filter((_, i) => i !== calcIndex)
                            )
                          }
                        >
                          Remove Calculation
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                        ...
                        <div className="flex flex-col">
                          <label
                            htmlFor={`calc-vg-${calcIndex}`}
                            className="text-sm font-medium mb-1"
                          >
                            Source Value Group
                          </label>
                          <Select
                            value={calc.value_group_selected}
                            onValueChange={(value) => {
                              const updatedCalcs = [...calculations];
                              updatedCalcs[calcIndex].value_group_selected =
                                value;
                              setCalculations(updatedCalcs);
                            }}
                          >
                            <SelectTrigger
                              id={`calc-vg-${calcIndex}`}
                              className="w-full"
                            >
                            ...
                            <SelectContent>
                              <SelectGroup>
                                {uniqueValueGroups.length === 0 ? (
                                  <SelectLabel>
                                    No value groups defined in devices.
                                  </SelectLabel>
                                ) : (
                                  uniqueValueGroups.map((vg, idx) => (
                                    <SelectItem key={idx} value={vg}>
                                      {vg}
                                    </SelectItem>
                                  ))
                                )}
                  ...
                  <div className="flex items-center mt-auto pt-2">
                    ...
                    />
                    <label
                      htmlFor="calculation-only-checkbox"
                      className="text-sm font-medium leading-none"
                    >
                ...
                <CardContent>
                  {previewPayload ? (
                    ...
                    <p className="text-muted-foreground text-sm">
                      Click "Generate Preview" to see the JSON payload that will
                      be sent to the middleware.
                    </p>
                  )}
                </CardContent>
              </Card>
