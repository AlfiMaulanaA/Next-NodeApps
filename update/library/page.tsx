...
export default function FileLibraryPage() {
  ...
  const [newDeviceVariables, setNewDeviceVariables] = useState<
    DeviceVariableEntry[]
  >([]);
  ...
  const [updateDeviceVariables, setUpdateDeviceVariables] = useState<
    DeviceVariableEntry[]
  >([]);
  const [selectedDeviceForUpdate, setSelectedDeviceForUpdate] =
    useState<any>(null);
  const [isLoadingForUpdate, setIsLoadingForUpdate] = useState<boolean>(false);
  const [isLoadingDeviceData, setIsLoadingDeviceData] =
  ...
  const organizedSections = useMemo(() => {
    ...
    tabs.Others = existingSections.filter(
      (section) => !predefinedSections.includes(section)
    );
  ...
  const availablePartNumbers = useMemo(() => {
    if (!updateSection || !updateManufacturer || !deviceSummary[updateSection])
      return [];
        ...
        if (topic === "response_file_transfer") {
          if (data.status === "success" && data.action === "upload") {
            ...
            if (clientRef.current) {
              ...
              console.warn(
                "ClientRef is null when trying to restart service after upload success."
              );
            }
          ...
          if (data.status === "success" && data.data) {
            ...
            if (isLoadingForUpdate) {
              ...
              if (data.data.data && Array.isArray(data.data.data)) {
                const processedVariables = data.data.data
                  ...
                  .map((variable: any) => {
                    ...
                    return {
                      ...
                      relative_address:
                        typeof variable.relative_address === "number"
                          ? variable.relative_address
                          : variable.relative_address || "",
                      register_type:
                        variable.register_type || "Holding Register",
                ...
                console.log(
                  "Processed variables for update:",
                  processedVariables
                ); // Debug log
              ...
              toast.success("Device data loaded for editing!", {
                id: "loadDeviceToast",
              });
            } else {
          ...
          if (data.status === "success" && data.data) {
            ...
            toast.error(
              data.message || "Failed to load device data for editing."
            );
          }
  ...
  const confirmUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (
      confirm(
        "Upload and replace devices.json file? This will restart the MODBUS_SNMP service."
      )
    ) {
      uploadFile(file);
    }
  ...
  const downloadFile = () => {
    ...
    toast.loading("Requesting devices.json download...", {
      id: "downloadToast",
    });
    const cmd = JSON.stringify({
      action: "download",
    ...
    clientRef.current.publish("command_download_file", cmd, (err) => {
      if (err) {
        toast.error(`Failed to send download command: ${err.message}`, {
          id: "downloadToast",
        });
        console.error("Publish error:", err);
      }
  ...
  const uploadFile = (file: File) => {
    ...
    reader.onload = () => {
      ...
      clientRef.current?.publish("command_upload_file", cmd, (err) => {
        if (err) {
          toast.error(`Failed to send upload command: ${err.message}`, {
            id: "uploadToast",
          });
          console.error("Publish error:", err);
        } else {
  ...
  const handleDeleteSection = () => {
    ...
    if (
      !confirm(
        `Are you sure you want to delete section "${sectionToDelete}"? This action cannot be undone.`
      )
    ) {
      return;
    }
  ...
  const handleSearchDevice = () => {
    ...
    if (searchManufacturer.trim())
      searchParams.manufacturer = searchManufacturer.trim();
    if (searchPartNumber.trim())
      searchParams.part_number = searchPartNumber.trim();
  ...
  const handleLoadDeviceForUpdate = () => {
    if (
      ...
      toast.error(
        "Please select section, manufacturer, part number, and protocol first."
      );
      return;
    }
    ...
    clientRef.current.publish(TOPIC_LIBRARY_EDIT_LOAD_COMMAND, cmd, (err) => {
      if (err) {
        ...
        toast.loading("Loading device data for editing...", {
          id: "loadDeviceToast",
        });
  ...
  const handleUpdateDevice = () => {
    ...
    if (
      ...
      toast.error(
        "Device selection is incomplete. Please load a device first."
      );
      return;
    }
    ...
    const formattedDeviceData = updateDeviceVariables.map((variable) => {
      const parsedAddress =
        typeof variable.relative_address === "string"
          ? parseInt(variable.relative_address, 10)
          : variable.relative_address;

      if (isNaN(parsedAddress as number) || variable.var_name.trim() === "") {
        toast.error(
          "All 'Var Name' and 'Relative Address' fields in Device Data must be filled correctly."
        );
  ...
  const handleAddNewDevice = () => {
    ...
    if (
      ...
      !newDevicePartNumber.trim() ||
      !newDeviceProtocol.trim()
    ) {
      toast.error(
        "Section, Manufacturer, Part Number, and Protocol are required to add a device."
      );
      return;
    }
        ...
        <Tabs defaultValue="library" className="w-full">
              ...
              <CardContent>
                {Object.keys(deviceSummary).length > 0 ? (
                        ...
                        >
                          <div className="max-h-80 overflow-y-auto space-y-4">
                            {sectionList.length > 0 ? (
                              sectionList
                                .filter(
                                  (sectionName) => deviceSummary[sectionName]
                                ) // Only show sections that exist in data
                                .map((sectionName) => {
                                      ...
                                      <ul className="list-disc pl-5 space-y-1 text-sm">
                                        {Array.isArray(devices) ? (
                                          devices.map((device, deviceIndex) => (
                                            <li
                                              key={`${sectionName}-${device.part_number}-${deviceIndex}`}
                                            >
                                          ...
                                          <li className="text-red-500">
                                            Error: Invalid data format for this
                                            section. Expected an array of
                                            devices.
                                          </li>
                                        )}
                                      </ul>
                  ...
                  <p className="text-muted-foreground text-sm">
                    No device summary available or loading... Ensure the backend
                    service is running and publishing updates to `
                    {TOPIC_LIBRARY_DEVICES_SUMMARY}`.
                  </p>
                )}
                <Button
                  onClick={() => {
                    /* This button currently just reloads page. Backend sends updates periodically */
                  }}
                  className="mt-4"
                  variant="outline"
                  size="sm"
                  disabled={true}
                >
                  <RotateCw className="mr-2 h-4 w-4" /> Summary updates
                  periodically
                </Button>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <Button
                  ...
                  disabled={status !== "connected" || !searchSection.trim()}
                >
                  ...
                  <div className="mt-4">
                    ...
                    {searchResult.status === "success" && searchResult.data ? (
                      <div className="bg-muted p-3 rounded-md text-sm max-h-40 overflow-y-auto">
                        {searchResult.data.manufacturer &&
                        searchResult.data.part_number ? (
                          <div className="mb-2">
                                      ...
                                      <ul className="space-y-1">
                                        {searchResult.data.data.map(
                                          (variable: any, index: number) => (
                                            <li
                                              key={index}
                                              className="text-xs border-b border-border/20 pb-1 last:border-b-0"
                                            >
                                              <div className="flex flex-wrap gap-2">
                                                <span>
                                                  <strong>Name:</strong>{" "}
                                                  {variable.var_name || "N/A"}
                                                </span>
                                                <span>
                          ...
                          <p className="text-muted-foreground">
                            No specific device data found in the successful
                            response.
                          </p>
                        )}
                      </div>
          ...
          <TabsContent value="manage" className="space-y-6 mt-6">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                {/* Create New Section */}
                <div>
                  <Label htmlFor="new-section-name" className="mb-2 block">
                    Create New Section
                  </Label>
                  <div className="flex gap-2">
                    ...
                    <Button
                      ...
                      disabled={
                        status !== "connected" || !newSectionName.trim()
                      }
                    >
                ...
                {/* Delete Section */}
                <div>
                  <Label htmlFor="delete-section-select" className="mb-2 block">
                    Delete Section
                  </Label>
                  <div className="flex gap-2">
                    <Select
                    ...
                    <Button
                      ...
                      disabled={
                        status !== "connected" || !sectionToDelete.trim()
                      }
                    >
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <p className="text-sm text-muted-foreground">
                  Add a new device by providing its details and custom data
                  variables.
                </p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <Label htmlFor="new-device-section">
                      Section (Required)
                    </Label>
                    <Select
                  ...
                  <div>
                    <Label htmlFor="new-device-manufacturer">
                      Manufacturer (Required)
                    </Label>
                    <Input
                      id="new-device-manufacturer"
                  ...
                  <div>
                    <Label htmlFor="new-device-part-number">
                      Part Number (Required)
                    </Label>
                    <Input
                      id="new-device-part-number"
                      value={newDevicePartNumber}
                      onChange={(e) => setNewDevicePartNumber(e.target.value)}
                      placeholder="e.g., TM221CE16R"
                      disabled={status !== "connected"}
                    />
                  </div>
                  <div>
                    <Label htmlFor="new-device-protocol">
                      Protocol (Required)
                    </Label>
                    <Input
                      id="new-device-protocol"
                ...
                <div className="space-y-3 p-4 border rounded-md bg-secondary/20">
                  ...
                  {newDeviceVariables.length === 0 && (
                    <p className="text-muted-foreground text-sm">
                      Click "Add Variable" to define data points for this
                      device.
                    </p>
                  )}
                  {newDeviceVariables.map((variable, index) => (
                    <div
                      key={index}
                      className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-3 border p-3 rounded-md bg-background relative"
                    >
                      <Button
                      ...
                      <div className="md:col-span-1">
                        <Label htmlFor={`relative_address-${index}`}>
                          Relative Address
                        </Label>
                        <Input
                      ...
                      <div className="md:col-span-1">
                        <Label htmlFor={`register_type-${index}`}>
                          Register Type
                        </Label>
                        <Select
                        ...
                        >
                          ...
                          <SelectContent>
                            <SelectItem value="Holding Register">
                              Holding Register
                            </SelectItem>
                            <SelectItem value="Input Register">
                              Input Register
                            </SelectItem>
                            <SelectItem value="Coil">Coil</SelectItem>
                            <SelectItem value="Discrete Input">
                              Discrete Input
                            </SelectItem>
                          </SelectContent>
                        </Select>
                      ...
                      <div className="md:col-span-1">
                        <Label htmlFor={`word_length-${index}`}>
                          Word Length
                        </Label>
                        <Input
                      ...
                      <div className="md:col-span-1">
                        <Label htmlFor={`multiplier-${index}`}>
                          Multiplier
                        </Label>
                        <Input
                          id={`multiplier-${index}`}
          ...
          <TabsContent value="update" className="space-y-6 mt-6">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <p className="text-sm text-muted-foreground">
                  Select a device by choosing section, manufacturer, part
                  number, and protocol. Then load the device data for editing.
                </p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <Label htmlFor="update-manufacturer">
                      Manufacturer (Required)
                    </Label>
                    <Select
                    ...
                    >
                      ...
                      <SelectContent>
                        {availableManufacturers.length > 0 ? (
                          ...
                          <SelectItem
                            value="no-manufacturers-available"
                            disabled
                          >
                            {updateSection
                              ? "No manufacturers in this section"
                  ...
                  <div>
                    <Label htmlFor="update-part-number">
                      Part Number (Required)
                    </Label>
                    <Select
                      ...
                      <SelectContent>
                        {availablePartNumbers.length > 0 ? (
                          ...
                          <SelectItem value="no-parts-available" disabled>
                            {updateManufacturer
                              ? "No parts for this manufacturer"
                        ...
                        <DialogDescription className="text-left space-y-4">
                          <div className="bg-blue-50 p-4 rounded-md border border-blue-200">
                            <h4 className="font-semibold text-blue-800 mb-2">
                              How to Update Device Library:
                            </h4>
                            <ol className="list-decimal list-inside space-y-2 text-sm text-blue-700">
                              <li>
                                <strong>Select Section:</strong> Choose the
                                device category from the dropdown (e.g., ups,
                                pdu, power_meter)
                              </li>
                              <li>
                                <strong>Select Manufacturer:</strong> Pick the
                                device manufacturer from available options in
                                that section
                              </li>
                              <li>
                                <strong>Select Part Number:</strong> Choose the
                                specific part number for that manufacturer
                              </li>
                              <li>
                                <strong>Select Protocol:</strong> Pick the
                                communication protocol (e.g., Modbus RTU, SNMP)
                              </li>
                              <li>
                                <strong>Load Device Data:</strong> Click the
                                "Load Device Data" button to fetch current
                                device variables
                              </li>
                              <li>
                                <strong>Edit Variables:</strong> Modify, add, or
                                remove device variables as needed using the form
                                below
                              </li>
                              <li>
                                <strong>Save Changes:</strong> Click "Update
                                Device" to save your modifications to the
                                library
                              </li>
                            </ol>
                          </div>
                          ...
                          <div className="bg-green-50 p-4 rounded-md border border-green-200">
                            ...
                            <ul className="list-disc list-inside space-y-1 text-sm text-green-700">
                              <li>
                                <strong>Var Name:</strong> Descriptive name for
                                the data point
                              </li>
                              <li>
                                <strong>Relative Address:</strong> Memory
                                address offset for the variable
                              </li>
                              <li>
                                <strong>Register Type:</strong> Modbus register
                                type (Holding, Input, Coil, Discrete)
                              </li>
                              <li>
                                <strong>Word Length:</strong> Number of 16-bit
                                words the variable occupies
                              </li>
                              <li>
                                <strong>Data Type:</strong> Variable data format
                                (UINT16, INT16, FLOAT32, etc.)
                              </li>
                              <li>
                                <strong>Multiplier:</strong> Scaling factor
                          ...
                          <div className="bg-yellow-50 p-4 rounded-md border border-yellow-200">
                            ...
                            <ul className="list-disc list-inside space-y-1 text-sm text-yellow-700">
                              <li>
                                Loading device data may take 1-5 seconds
                                depending on library size
                              </li>
                              <li>
                                All dropdowns are cascading - selecting one
                                field affects the next
                              </li>
                              <li>
                              ...
                              <li>
                                Changes are saved immediately to the device
                                library file
                              </li>
                              <li>
                                Make sure MQTT connection is active before
                                making changes
                              </li>
                            </ul>
                          </div>
                ...
                {selectedDeviceForUpdate && (
                  <div className="mt-4 p-3 bg-muted rounded-md">
                    ...
                    <p className="text-sm">
                      ...
                      {selectedDeviceForUpdate.section} |
                      <strong> Manufacturer:</strong>{" "}
                      {selectedDeviceForUpdate.manufacturer} |
                      <strong> Part Number:</strong>{" "}
                      {selectedDeviceForUpdate.part_number} |
                      <strong> Protocol:</strong>{" "}
                      {selectedDeviceForUpdate.protocol}
                    </p>
                  </div>
                )}
              </CardContent>
            ...
            {selectedDeviceForUpdate && (
              <Card>
                ...
                <CardContent className="space-y-4">
                  <p className="text-sm text-muted-foreground">
                    Edit the device variables below. You can add, remove, or
                  ...
                  <div className="space-y-3 p-4 border rounded-md bg-secondary/20">
                    ...
                    {updateDeviceVariables.length === 0 && (
                      <p className="text-muted-foreground text-sm">
                        No variables defined. Click "Add Variable" to add data
                        points.
                      </p>
                    )}
                    {updateDeviceVariables.map((variable, index) => (
                      ...
                      >
                        <Button
                          onClick={() =>
                            handleRemoveUpdateDeviceVariable(index)
                          }
                          type="button"
                          variant="ghost"
                          size="icon"
                        ...
                        <div className="md:col-span-1">
                          <Label htmlFor={`update_var_name-${index}`}>
                            Var Name
                          </Label>
                          <Input
                            ...
                            value={variable.var_name}
                            onChange={(e) =>
                              handleUpdateDeviceVariableChange(
                                index,
                                "var_name",
                                e.target.value
                              )
                        ...
                        <div className="md:col-span-1">
                          <Label htmlFor={`update_relative_address-${index}`}>
                            Relative Address
                          </Label>
                          <Input
                        ...
                        <div className="md:col-span-1">
                          <Label htmlFor={`update_register_type-${index}`}>
                            Register Type
                          </Label>
                          <Select
                            value={variable.register_type}
                        ...
                        <div className="md:col-span-1">
                          <Label htmlFor={`update_word_length-${index}`}>
                            Word Length
                          </Label>
                          <Input
                        ...
                        <div className="md:col-span-1">
                          <Label htmlFor={`update_data_type-${index}`}>
                            Data Type
                          </Label>
                          <Select
                            value={variable.data_type}
                        ...
                        <div className="md:col-span-1">
                          <Label htmlFor={`update_multiplier-${index}`}>
                            Multiplier
                          </Label>
                          <Input
                  ...
                  <div className="flex justify-between">
                    <Button
                      ...
                      disabled={
                        status !== "connected" || !selectedDeviceForUpdate
                      }
                    >
