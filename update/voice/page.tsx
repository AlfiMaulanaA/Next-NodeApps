...
const VoiceControlPage = () => {
  ...
  const [filteredRelayDevices, setFilteredRelayDevices] = useState<
    ModularDevice[]
  >([]);
  ...
  const openModal = (item?: VoiceControl) => {
    if (item) {
      setIsEditing(true);
      // Normalize uuid from backend to id field
      const itemWithId = {
        ...item,
        id: (item as any).uuid || item.id, // Use uuid from backend if it exists
      };
      setSelectedControl(itemWithId.id);
      setVoiceControl(itemWithId);
    } else {
      setIsEditing(false);
  ...
  useEffect(() => {
    ...
    const handleVoiceControlData = (topic: string, message: Buffer) => {
      try {
        ...
        // Normalize incoming data: convert uuid to id
        const normalizedData = Array.isArray(payload)
          ? payload.map((item: any) => ({
              ...item,
              id: item.uuid || item.id, // Use uuid from backend if it exists
            }))
          : [];
        setVoiceControlData(normalizedData);
        console.log("MQTT: Data voice control diterima:", normalizedData);
      } catch (error) {
    ...
    const handleModularData = (topic: string, message: Buffer) => {
      try {
        ...
        if (payload.status === "success" && payload.data) {
          console.log(
            "MQTT: Modular devices (wrapped format):",
            payload.data.length,
            payload.data
          );
          ...
          console.log(
            "MQTT: Modular devices (direct array):",
            payload.length,
            payload
          );
  ...
  const saveVoiceControl = (e: React.FormEvent) => {
    ...
    // Normalize data for backend: convert 'id' to 'uuid'
    const dataToSend = {
      ...voiceControl,
      uuid: voiceControl.id, // Convert id to uuid for backend
    };
    delete (dataToSend as any).id; // Remove id field since backend uses uuid
  ...
  const deleteVoiceControl = (uuid: string) => {
    showConfirmation({
      ...
      onConfirm: () => {
        ...
        const message = {
          action: "delete",
          data: { uuid: uuid }, // Send only uuid, backend uses this field
        };
  ...
  const processVoiceCommand = useCallback(
    (command: string) => {
      ...
      if (
        lowerCommand.includes("nyalakan") ||
        lowerCommand.includes("hidupkan")
      ) {
        action = 1;
        objectName = lowerCommand.replace(/nyalakan|hidupkan/gi, "").trim();
      } else if (
        lowerCommand.includes("matikan") ||
        lowerCommand.includes("mati")
      ) {
        action = 0;
      ...
      const matchedControl = voiceControlData.find((ctrl) => {
        ...
        return (
          ctrlRuleName.includes(objectName) || objectName.includes(ctrlRuleName)
        );
      });
      ...
      if (mqttClient && isConnected) {
        mqttClient.publish("modular", JSON.stringify(payload), (err) => {
          if (err) {
            ...
            showToast.success(
              "Command Sent",
              `${action === 1 ? "Turning ON" : "Turning OFF"} ${
                matchedControl.rule_name
              }`
            );
          }
        });
    ...
    <SidebarInset>
      ...
      <header className="flex h-16 items-center justify-between border-b px-4">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <Mic
              className={`h-4 w-4 mr-2 ${isListening ? "animate-bounce" : ""}`}
            />
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    >
                      ...
                      <SelectContent>
                        {Array.from(
                          { length: getMaxPins(voiceControl.device_name) },
                          (_, i) => (
