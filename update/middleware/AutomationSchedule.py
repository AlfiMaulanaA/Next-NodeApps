...
client_error_logger = None # Dedicated client for sending error logs to 18.143.215.113
...
# --- Error Log Helper Function (to 18.143.215.113) ---
...
broker_control = mqtt_config.get('broker_address', '18.143.215.113')
...
def load_config():
    ...
    try:
        ...
        if isinstance(loaded_data, list):
            ...
            log_simple(f"Loaded {len(config)} scheduled devices", "SUCCESS")
            for device in config:
                device_name = device.get('customName', device.get('name', device.get('id', 'Unknown')))
                controls_count = len(device.get('controls', []))
                log_simple(f"  ├─ {device_name} ({controls_count} schedules)", "INFO")
            ...
            log_simple(f"Loaded {len(config)} scheduled devices (from object format)", "SUCCESS")
        else:
            config = []  # Default empty array
            logger.warning("Config format not recognized, initializing as empty array.")
            send_error_log("load_config", f"Invalid config format - expected list or dict with 'devices', got {type(loaded_data).__name__}", ERROR_TYPE_WARNING, {"file": config_file})
    except FileNotFoundError:
        logger.warning(f"Config file not found: {config_file}. Creating a default config.")
        config = []  # Default empty array
        save_config(config) # Create the file
        send_error_log("load_config", f"Scheduler config file not found at {config_file}, created new", ERROR_TYPE_WARNING, {"file": config_file})
    except json.JSONDecodeError as e:
        send_error_log("load_config", f"JSON decode error in scheduler config: {str(e)}", ERROR_TYPE_MAJOR, {"file": config_file, "line": getattr(e, 'lineno', 'unknown')})
        ...
        send_error_log("load_config", f"Failed to load scheduler config: {e}", ERROR_TYPE_MAJOR, {"file": config_file})
...
def load_installed_devices():
    ...
    try:
        ...
        log_simple(f"Loaded {len(installed_devices)} installed devices", "SUCCESS")
        ...
        installed_error_detail = f"Installed devices file not found at {installed_devices_file}"
        send_error_log("load_installed_devices", installed_error_detail, ERROR_TYPE_WARNING, {"file": installed_devices_file})
        installed_devices = []
    except json.JSONDecodeError as e:
        send_error_log("load_installed_devices", f"JSON decode error loading installed devices: {str(e)}", ERROR_TYPE_MAJOR, {"file": installed_devices_file, "line": getattr(e, 'lineno', 'unknown')})
...
def save_config(current_config):
    try:
        ...
        log_simple(f"Saved {len(current_config)} scheduled devices", "SUCCESS")
    except IOError as e:
        send_error_log("save_config", f"I/O error saving scheduler config: {e}", ERROR_TYPE_MAJOR, {"file": config_file})
        logger.error(f"Failed to save config (I/O error): {e}")
    except Exception as e:
        send_error_log("save_config", f"Failed to save scheduler config: {e}", ERROR_TYPE_MAJOR, {"file": config_file, "device_count": len(current_config)})
...
def modify_config(client, message):
    ...
    device_name = device_data.get('customName', device_data.get('name', 'Unknown')) if device_data else 'Unknown'
    ...
    try:
        ...
        if action == 'add' and not device_data.get('id'):
            ...
            log_simple(f"Generated new device ID for add operation: {device_id}", "INFO")
        elif action != 'add' and not device_data.get('id'):
            send_error_log("modify_config", f"Device ID missing for {action} action", ERROR_TYPE_WARNING, {"action": action, "device_name": device_name})
        ...
        if action == 'set':
            ...
            if index is not None:
                controls_count = len(device_data.get('controls', []))
                config[index] = device_data
                logger.info(f"Updated device with ID: {device_data['id']}")
                log_simple(f"Updated scheduled device: {device_name} ({controls_count} schedules)", "SUCCESS")
            else:
                ...
                send_error_log("modify_config", f"Device not found for update (set action)", ERROR_TYPE_WARNING, {"action": action, "device_id": device_data['id'], "device_name": device_name})
                return
        elif action == 'add':
            ...
            log_simple(f"Added scheduled device: {device_name} ({controls_count} schedules)", "SUCCESS")
        elif action == 'delete':
            ...
            if len(config) < initial_len:
                ...
                log_simple(f"Deleted scheduled device: {device_name}", "SUCCESS")
            else:
                ...
                send_error_log("modify_config", f"Device not found for deletion (delete action)", ERROR_TYPE_WARNING, {"action": action, "device_id": device_data['id'], "device_name": device_name})
                return
        else:
...
def schedule_control(client_control):
    try:
        if not config or not isinstance(config, list) or len(config) == 0:
            ...
            send_error_log("schedule_control", "No devices configured for scheduling", ERROR_TYPE_WARNING)
            return
        ...
        for device in config:
            ...
            if 'id' not in device or 'startDay' not in device or 'endDay' not in device or 'controls' not in device:
                device_name = device.get('customName', device.get('name', device.get('id', 'unknown_id')))
                logger.warning(f"Skipping malformed device entry in config: {device_name}")
                send_error_log("schedule_control", f"Malformed device entry - missing required fields", ERROR_TYPE_WARNING, {"device_name": device_name, "device_id": device.get('id', 'unknown')})
                continue
            ...
            if not isinstance(device['controls'], list):
                ...
                send_error_log("schedule_control", f"Device has malformed controls - not a list", ERROR_TYPE_WARNING, {"device_id": device['id'], "device_name": device_name})
                continue
...
def send_control_signal(client, device, pin, data):
    ...
    if not device.get('autoControl', True):
        logger.info(f"Auto control is disabled for device. Not sending signal to {device_name}, pin {pin}, data {data}.")
        send_error_log("send_control_signal", f"Auto control disabled for device", ERROR_TYPE_WARNING, {"device_name": device_name, "device_id": device.get('id'), "pin": pin})
        return

    try:
        # Basic validation for critical device info
        if not all(key in device for key in ['part_number', 'address', 'device_bus']):
            missing_keys = [key for key in ['part_number', 'address', 'device_bus'] if key not in device]
            logger.error(f"Missing critical device information for sending control signal: {device_name}")
            send_error_log("send_control_signal", f"Missing critical device info: {', '.join(missing_keys)}", ERROR_TYPE_CRITICAL, {"device_name": device_name, "device_id": device.get('id', 'unknown'), "missing_keys": missing_keys})
            return
        ...
        logger.info(f"Sent control signal to {device_name}, pin {pin}, data {data} (using active MAC: {active_mac})")
        log_simple(f"Control signal sent: {device_name} pin {pin} → {bool(data)}", "SUCCESS")
    except Exception as e:
        send_error_log("send_control_signal", f"Failed to send control signal: {e}", ERROR_TYPE_CRITICAL, {"device_name": device_name, "device_id": device.get('id', 'unknown'), "pin": pin, "data": data})
