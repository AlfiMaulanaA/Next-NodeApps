...
export default function SNMPGetPage() {
  ...
  useEffect(() => {
    ...
    mqttClientInstance.subscribe(
      SNMP_DATA_RESPONSE_TOPIC,
      { qos: 0 },
      (err) => {
        if (err)
          console.error(
            `Failed to subscribe to ${SNMP_DATA_RESPONSE_TOPIC}:`,
            err
          );
      }
    );
  ...
  const getPaginationPages = () => {
    ...
    if (totalPages <= maxVisible) {
      ...
      const rangeStart = Math.max(2, currentPage - sidesVisible);
      const rangeEnd = Math.min(totalPages - 1, currentPage + sidesVisible);
  ...
  const copyToClipboard = async (oid: string) => {
    try {
      ...
      try {
        ...
        showToast.success(
          "OID Copied",
          `${oid} has been copied to clipboard (fallback method)`
        );
      } catch (fallbackErr) {
        showToast.error(
          "Copy Failed",
          "Unable to copy OID to clipboard. Please copy manually."
        );
    ...
    client.publish(
      ...
      (err) => {
        if (err) {
          ...
          toast.error(
            `Failed to send SNMP ${operation} command: ${err.message}`
          );
          setIsLoading(false);
        } else {
            ...
            <CardContent>
              <Tabs defaultValue="table" className="w-full">
                              ...
                              >
                                <td className="p-2">
                                  <div className="flex items-center gap-2">
                                    <Badge
                                      variant="outline"
                                      className="font-mono text-xs bg-muted text-muted-foreground border-border"
                                    >
                                      {result.oid}
                                    </Badge>
                                    <Button
                                ...
                                <td className="p-2 font-mono break-all text-sm">
                                  {String(result.value)}
                                </td>
                              </tr>
                            ))}
                ...
                <TabsContent value="json">
                  <div className="bg-muted p-4 rounded border border-border overflow-hidden">
                    <pre className="text-xs overflow-x-auto overflow-y-auto max-h-[500px] text-muted-foreground font-mono">
